...
export async function syncAllGuilds(client: Client) {
	...
	for (const guild of guilds) {
		...
		await syncMembers(guild[1].id, client)
	}
...
export async function syncMembers(guildId: string, client: Client) {
	const [logger] = await resolveDependencies([Logger])
	const guild = await client.guilds.fetch(guildId);
	logger.logChalk(
		`(MEMBER_SYNC) Syncing members of ${guild.name} (${guild.id})`,
	 	`(${chalk.bold.white('MEMBER_SYNC')}) ${chalk.bold.green('Syncing members of')} ${chalk.bold.blue(guild.name)} (${chalk.dim.italic.gray(guild.id)})`
	)
